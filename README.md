...
專案使用 `pyproject.toml` 與 `uv.lock` 管理依賴，並透過 `tool.uv.index` 指向 CUDA 13.0 的 PyTorch wheel。`uv` 預設會建立專案內的 `.venv/`，這也符合目前 Kinect DLL 搜尋邏輯。若要在沒有 NVIDIA GPU 的環境執行，請改裝符合環境的 `torch` / `torchvision` / `onnxruntime` 版本。
