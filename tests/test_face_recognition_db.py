...
def _write_embeddings(face_db, entries):
    ...
    face_db.embedding_file.write_text(json.dumps({"students": entries}, ensure_ascii=False), encoding="utf-8")
...
class TestCosineSimilarity:
    ...
    def test_orthogonal_vectors(self, face_db):
        assert face_db._cosine_similarity(_vector(1.0, 0.0), _vector(0.0, 1.0)) == 0.0

    def test_opposite_vectors(self, face_db):
        assert face_db._cosine_similarity(_vector(1.0, 0.0), _vector(-1.0, 0.0)) == -1.0

    def test_zero_vector(self, face_db):
        assert face_db._cosine_similarity(_vector(1.0, 0.0), _vector(0.0, 0.0)) == 0.0
...
class TestMatchEmbedding:
    ...
    def test_sorted_by_similarity_descending(self, face_db):
        ...
        assert [r["label"] for r in result] == ["\u5b78\u751f3", "\u5b78\u751f1", "\u5b78\u751f2"]
...
class TestLoadDatabase:
    ...
    def test_cache_invalidated_on_mtime_change(self, face_db):
        ...
        os.utime(face_db.embedding_file, (face_db.embedding_file.stat().st_mtime + 1,) * 2)
        second = face_db.load_database()
...
class TestResolveProfileLabel:
    def test_matches_by_student_id(self, face_db):
        ...
        label, desired = face_db._resolve_profile_label(profiles, "001", "Alice")
        assert label == "\u5b78\u751f1"
    ...
    def test_matches_by_name(self, face_db):
        ...
        label, desired = face_db._resolve_profile_label(profiles, "", "Alice")
        assert label == "\u5b78\u751f1"
        assert desired == "Alice"
...
class TestListStudentLabels:
    def test_returns_sorted_dir_names(self, face_db):
        ...
        assert face_db.list_student_labels() == ["\u5b78\u751f1", "\u5b78\u751f10", "\u5b78\u751f2"]
...
class TestSaveTrainingImage:
    def test_unknown_label_raises(self, face_db):
        ...
        with pytest.raises(RuntimeError, match="Unknown student label"):
            face_db.save_training_image("\u4e0d\u5b58\u5728", "test.jpg", b"fake_image_data")

    def test_saves_file_with_proper_label(self, face_db):
        face_db.get_photo_root()
        (face_db.photo_root / "\u5b78\u751f1").mkdir()
        result = face_db.save_training_image("\u5b78\u751f1", "test.jpg", b"fake_image_data")
